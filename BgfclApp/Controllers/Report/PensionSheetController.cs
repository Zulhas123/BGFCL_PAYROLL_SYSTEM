...
{
    public class PensionSheetController : Controller
    {
        ...
        public PensionSheetController(IWebHostEnvironment webHostEnvironment,
        ...
        //**************** Pension Sheet Report Start ****************

        [HttpGet]
        public async Task<IActionResult> GetPensionSheet()
        {
        ...
        [HttpPost]
        public async Task<IActionResult> GetPensionSheet(List<string> jobCode, int month, int year, string employeeType, string department)
        {
            var monthId = year * 100 + month;
            var source = await _pFSheetReportContract.GetPFSheetOF(jobCode, monthId, department);
            string mimetype = "";
            int extension = 1;
            var path = $"{_webHostEnvironment.WebRootPath}\\Reports\\RptPensionSheet.rdlc";
            Dictionary<string, string> parameters = new Dictionary<string, string>();
            LocalReport localReport = new LocalReport(path);

            localReport.AddDataSource("dsPensionSheet", source);
