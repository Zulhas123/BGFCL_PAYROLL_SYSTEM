...
$(document).ready(function () {
    ...
    // Fetch the employee list and populate the filter dropdown
    $.ajax({
    ...
    $('#filter_select').on('change', function () {
        ...
        if (selectedEmployee) {
            getEmployeeList(selectedEmployee);
        } else {
            getEmployeeList();
        }
    });
        ...
        if (departmentId > 0) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#JournalCode').val(departmentData.journalCode);
                }
            });
        } else {
            $('#JournalCode').val('');
...
function loadInitialData() {
    ...
    $.ajax({
        url: '/api/Grades/GetGradesByEmployeeType?employeeTypeId=3',
        type: 'Get',
        async: false,
        dataType: 'json',
        success: function (responseObject) {
    ...
    $.ajax({
        url: '/api/Designations/GetDesignationsByEmployeeType?employeeTypeId=3',
        type: 'Get',
        async: false,
        dataType: 'json',
        success: function (responseObject) {
...
function getEmployeeList(pageLength) {
    ...
    _dataTable = $('#employee_list_table').DataTable({
        ...
        columns: [
            {
                data: '', className: 'text-center align-middle', render: function (data, type, row, meta) {
            ...
            {
                data: 'id', orderable: false, className: 'text-center align-middle', render: function (data) {
...
function onEditClicked(employeeId) {
    ...
    $.ajax({
        ...
        if (responseObject.statusCode == 200) {
            ...
            $('#Nid').val(responseObject.data.nid);
