...
$(document).ready(function () {
    ...
    $('#submit_button').on('click', function () {
        ...
        let employeeTypeId = $('#EmployeeTypeId').val();
            $.ajax({
                url: `/api/SalarySettings/JournalAdjustmentOfficer?monthId=${_monthId}&employeeTypeId=${employeeTypeId}`,
                type: 'GET',
                dataType: 'json',
                        ...
                        if (responseObject.data.length > 0) {
                            jss = $('#spreadsheet').jspreadsheet({
                                ...
                                columns: [
                                    ...
                                    { type: 'text', title: 'Journal Code', width: 150, name: 'journalCode' },
                            ...
                            $('#spreadsheet').on('click', '.delete-row', function () {
                                ...
                                if (rowIdx !== null && rowIdx !== undefined && rowIdx >= 0) {
                                    ...
                                    if (rowId) {
                                        $.ajax({
                                            url: `/api/SalarySettings/DeleteSalaryjournalOfficer?id=` + rowId,
    ...
    $('#update_button').on('click', function () {
        ...
        if (validRowData.length > 0) {
            $.ajax({
                url: `/api/SalarySettings/InsertJournalAdjustmentOfficer`,
                type: 'POST',
                async: false,
                contentType: 'application/json',
        ...
        if (updatedRow.length > 0) {
            ...
            $.ajax({
                url: `/api/SalarySettings/UpdateJournalAdjustmentOfficer`,
                contentType: 'application/json',
                dataType: 'json',
                type: 'POST',
                async: false,
